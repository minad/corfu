...
Corfu is available from [[https://elpa.gnu.org/packages/corfu.html][ELPA]] or [[https://melpa.org/#/corfu][MELPA]] and can be installed via =M-x package-install RET corfu RET=.
...
you install the package. The extensions are inactive by default and can be
enabled manually if desired. Furthermore it is possible to install all of the
