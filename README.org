...
** Same key used for both the separator and the insertion
...
Now you can type "spc" to insert a separator, and "spc spc" to insert the current candidate.
Moreover, if you do C-u and then separator, it leaves the word as it is, without completion, and inserts a space.
