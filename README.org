...
+ Annotations are not supported, but could be added
  ...
  considers verbose annotations like documentation strings or annotations in the
  style of [[https://github.com/minad/marginalia][Marginalia]]. These annotations work better in Vertico or in the
  =*Completions*= buffer.
...
+ Company enhancements are not supported (~company-docsig~, ~company-location~, ...).
